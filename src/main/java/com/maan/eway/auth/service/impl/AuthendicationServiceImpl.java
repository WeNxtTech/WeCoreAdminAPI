...
//				session.setTempTokenid(temptoken);
				...
				session.setIpAddress(http.getLocalAddr() +":" +http.getLocalPort());
		...
		try {
			...
			List<LoginProductMaster> loginproduct = new ArrayList<LoginProductMaster>();//loginProductRepo.findByLoginId(login.getLoginId());
			{
				...
				effectiveDate.select(
					    cb.greatest(ocpm1.<Timestamp>get("effectiveDateStart"))
					);
				...
				effectiveDate2.select(
					    cb.greatest(ocpm2.<Timestamp>get("effectiveDateEnd"))
					);
				...
				effectiveDate3.select(
					    cb.greatest(ocpm4.<Timestamp>get("effectiveDateStart"))
					);
				...
				effectiveDate4.select(
					    cb.greatest(ocpm5.<Timestamp>get("effectiveDateEnd"))
					);
	...
	private List<LoginBranchCriteriaRes> getBranchDetails(Set<String> removeDuplicateBranch) {
		...
		try {
			...
			effectiveDate2.select(
				    cb.greatest(ocpm2.<Timestamp>get("effectiveDateStart"))
				);
...
//			companyLogo.where(in1,in2);
			...
			effectiveDate6.select(
				    cb.greatest(ocpm6.<Timestamp>get("effectiveDateStart"))
				);
