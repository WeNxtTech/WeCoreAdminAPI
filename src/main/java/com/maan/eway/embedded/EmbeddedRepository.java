...
public interface EmbeddedRepository extends JpaRepository<GroupMedicalDetails, GroupMedicalDetailsId>{
	...
	@Query(value=" SELECT SUM(overall_premium) AS active_premium ,(SELECT item_value FROM eway_list_item_value WHERE item_type='PLAN_OPTED' AND STATUS='Y' AND SYSDATE() BETWEEN effective_date_start AND effective_date_end AND item_code=gmd.plan_opted AND (company_id=gmd.company_id OR company_id='99999') AND amend_id=(SELECT MAX(amend_id) FROM eway_list_item_value WHERE item_type='PLAN_OPTED' AND STATUS='Y' AND SYSDATE() BETWEEN effective_date_start AND effective_date_end AND item_code=gmd.plan_opted AND (company_id=gmd.company_id OR company_id='99999'))) AS plan_desc FROM group_medical_details gmd WHERE login_id=:loginId AND company_id =:companyId AND product_id=:productId AND plan_opted =:planOpted AND DATE(expiry_date) BETWEEN :startDate AND :endDate GROUP BY company_id,product_id,plan_opted",nativeQuery=true)
	public Map<String, Object> getActivePremiumBasedPlan(@Param("loginId") String loginId,@Param("companyId") String companyId,@Param("productId") String productId,@Param("planOpted") String planOpted,@Param("startDate") Date startDate, @Param("endDate") Date endDate);

	@Query(value="SELECT request_reference_no,policy_no,mobile_code,mobile_no,login_id,product_id,section_id,company_id,client_transaction_no,customer_name,plan_opted,amount_paid,premium,commission_percentage,commission_amount,tax_percentage,tax_premium,overall_premium FROM group_medical_details WHERE DATE(inception_date) BETWEEN :startDate  AND :endDate AND company_id=:companyId  AND product_id=:productId  AND login_id=:loginId  AND STATUS='Y' AND plan_opted='97' AND SYSDATE()>expiry_date",nativeQuery=true)
	public List<Map<String, Object>> getActivePolicy(@Param("loginId") String loginId,@Param("companyId") String companyId,@Param("productId") String productId , @Param("startDate") Date startDate, @Param("endDate") Date endDate);
...
=======
	@Query(value="SELECT request_reference_no,policy_no,mobile_code,mobile_no,login_id,product_id,section_id,company_id,client_transaction_no,customer_name,plan_opted,amount_paid,premium,commission_percentage,commission_amount,tax_percentage,tax_premium,overall_premium FROM group_medical_details WHERE DATE(inception_date) BETWEEN :startDate AND :endDate AND company_id=:companyId AND product_id=:productId AND login_id=:loginId AND STATUS='Y' AND plan_opted='97'",nativeQuery=true)
	public List<Map<String, Object>> getAllPolicy(@Param("loginId") String loginId,@Param("companyId") String companyId,@Param("productId") String productId , @Param("startDate") Date startDate, @Param("endDate") Date endDate);
>>>>>>> Stashed changes

	@Query(value="SELECT request_reference_no,policy_no,mobile_code,mobile_no,login_id,product_id,section_id,company_id,client_transaction_no,customer_name,plan_opted,amount_paid,premium,commission_percentage,commission_amount,tax_percentage,tax_premium,overall_premium FROM group_medical_details WHERE DATE(inception_date) BETWEEN :startDate AND :endDate AND company_id=:companyId AND product_id=:productId AND login_id=:loginId AND STATUS='Y' AND plan_opted='97' AND SYSDATE()<expiry_date",nativeQuery=true)
	public List<Map<String, Object>> getExpiredPolicy(@Param("loginId") String loginId,@Param("companyId") String companyId,@Param("productId") String productId , @Param("startDate") Date startDate, @Param("endDate") Date endDate);
}
