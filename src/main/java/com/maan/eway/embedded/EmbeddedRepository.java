...
public interface EmbeddedRepository extends JpaRepository<GroupMedicalDetails, GroupMedicalDetailsId>{
	...
	@Query(value="SELECT request_reference_no,policy_no,mobile_code,mobile_no,login_id,product_id,section_id,company_id,client_transaction_no,customer_name,plan_opted,amount_paid,premium,commission_percentage,commission_amount,tax_percentage,tax_premium,overall_premium FROM group_medical_details WHERE DATE(inception_date) BETWEEN :startDate  AND :endDate AND company_id=:companyId  AND product_id=:productId  AND login_id=:loginId  AND STATUS='Y' AND plan_opted=:planId AND expiry_date>=SYSDATE()",nativeQuery=true)
	public List<Map<String, Object>> getActivePolicy(@Param("loginId") String loginId,@Param("companyId") String companyId,@Param("productId") String productId , @Param("startDate") Date startDate, @Param("endDate") Date endDate,@Param("planId") String planId);
	...
	public List<Map<String, Object>> getAllPolicy(@Param("loginId") String loginId,@Param("companyId") String companyId,@Param("productId") String productId , @Param("startDate") Date startDate, @Param("endDate") Date endDate,@Param("planId") String planId);

	@Query(value="SELECT request_reference_no,policy_no,mobile_code,mobile_no,login_id,product_id,section_id,company_id,client_transaction_no,customer_name,plan_opted,amount_paid,premium,commission_percentage,commission_amount,tax_percentage,tax_premium,overall_premium FROM group_medical_details WHERE DATE(inception_date) BETWEEN :startDate AND :endDate AND company_id=:companyId AND product_id=:productId AND login_id=:loginId AND STATUS='Y' AND plan_opted=:planId AND expiry_date<SYSDATE()",nativeQuery=true)
	public List<Map<String, Object>> getExpiredPolicy(@Param("loginId") String loginId,@Param("companyId") String companyId,@Param("productId") String productId , @Param("startDate") Date startDate, @Param("endDate") Date endDate,@Param("planId") String planId);
}
