...
@Api(tags = "2. COMPANY CONFIG : Product Tax Setup ", description = "API's")
public class ProductTaxSetupController {

	@Autowired
	private  ProductTaxSetupService entityService;
	...
	public ResponseEntity<CommonRes> saveCompanyTaxes(@RequestBody ProductTaxSaveReq req) {
		...
		if (validation != null && validation.size() != 0) {
			...
			// Get All
			SuccessRes2 res = entityService.saveProductTaxes(req);
	...
	@PreAuthorize("hasAnyRole('ROLE_ADMIN','ROLE_APPROVER')")
	@PostMapping("/getallproducttax")
	@ApiOperation("This method is getall Product Taxes")
	public ResponseEntity<CommonRes> getallProductTaxes(@RequestBody GetAllProductTaxReq req)
	{
		CommonRes data = new CommonRes();
		
		ProductTaxGetRes res =entityService.getallProductTaxes(req);
		data.setCommonResponse(res);
		data.setErrorMessage(Collections.emptyList());
		data.setIsError(false);
		data.setMessage("Success");
