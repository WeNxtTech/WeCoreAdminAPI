...
import com.maan.eway.vehicleupload.VehicleCustomFieldSetMapper;

@Configuration
public class NonMotorRawDataBatchConfig {
	...
	private NonMotorItemWriter batchWriter =new NonMotorItemWriter();
	...
	@Autowired
	@Lazy
	@Qualifier("nonmot_stepexceution")
	private Step nonmot_stepexceution;

	
	@Bean("nonmot_raw_job")
	public Job nonmot_raw_job() {			
		return new JobBuilder("nonmot_raw_job",jobRepository)
				.start(nonmotraw_master_Step())
				.listener(listener())
				.build();
				
	}
	
	@Bean("nonmotraw_master_Step")
	public Step nonmotraw_master_Step() {
		return new StepBuilder("nonmotraw_master_Step",jobRepository)
				.partitioner(nonmot_stepexceution)
				.partitioner(nonmot_stepexceution.getName(), partitioner(0,0))
				.partitionHandler(partitionHandler())
				.build();
	}


	@Bean("nonmot_partitionHandler")
	public PartitionHandler partitionHandler() {
		 TaskExecutorPartitionHandler handler = new TaskExecutorPartitionHandler();
	        handler.setTaskExecutor(taskExecutor());
	        handler.setStep(nonmot_stepexceution);
	        return handler;
	  }

	@StepScope
	@Bean("nonmot_partitioner")
	...
	public TaskExecutor taskExecutor(){
		  asyncTaskExecutor=new ThreadPoolTaskExecutor();
		 	...
		 	asyncTaskExecutor.setThreadNamePrefix("nonmot_batch");
		 	asyncTaskExecutor.initialize();
	    return asyncTaskExecutor;
	}

	
	   @Bean("nonmot_stepexceution")
	   public Step nonmot_stepexceution() {
	         return new StepBuilder("nonmot_stepexceution",jobRepository)
	    ...
	    @Bean(name="nonmot_VehicleJobListener")
		public JobExecutionListener listener() {
			 return new NonMotJobExecutionListener();
		}    
