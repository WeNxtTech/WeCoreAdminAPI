...
public class UtilityServiceImpl {
	...
	public void convertToCsvFile(UtilityServiceImpl utilService, FileUploadInputRequest request) {
		...
		try {
			...
			// String csvFileName = request.get.replace(".xlsx", ".csv").replace(".xls",
			// ".csv").replace(".XLSX", ".csv").replace(".XLS", ".csv");
			String currentDateTime = sdf.format(new Date()).replaceAll("[^0-9]", "");
			...
			String factorTypeId = StringUtils.isBlank(sectionCov.get(0).getFactorTypeId().toString()) ? ""
					: sectionCov.get(0).getFactorTypeId().toString();
			Date effectiveDate = sectionCov.get(0).getEffectiveDateStart().toString() == null ? null
					: sectionCov.get(0).getEffectiveDateStart();
			...
			String createdBy = StringUtils.isBlank(sectionCov.get(0).getCreatedBy()) ? ""
					: sectionCov.get(0).getCreatedBy();
			...
			if (columnStatus) {
				...
				JobParameters jobParameters = new JobParametersBuilder().addLong("time", System.currentTimeMillis())
						.addString("ewayBatchData", print.toJson(request)).toJobParameters();
				Job processJob = applicationContext.getBean("ewayJobProcess", Job.class);
	...
	private Boolean checkMismatchedColumns(FileUploadInputRequest request) {
		...
		try {
			...
			Boolean columnLength = excelHeaders.length == request.getExcelHeaderColumns().split("~").length + 1 ? true
					: false;
			if (columnLength) {
				...
				boolean status = s1.toString().replaceAll("\\s", "").contentEquals(s2.toString().replaceAll("\\s", ""))
						? true
						: false;

				return true;
			} else {
				updateBatchTransaction(request.getTranId(), "checking mismatching columns",
						"Xl Heder columns is not matched", "Progressing", "E");
		...
		try {
			...
			for (int eachRow = 0; eachRow < rows; eachRow++) {
				...
				for (int i = 0; i < firstCount; i++) {
					...
					csvData = (String.valueOf(csvData) + getXLSXCellData(myRow.getCell(i)) + "~").replace("\t", "")
	...
	public int excelToCSV(String excelFileName, String csvFileName, FileUploadInputRequest request) {
		...
		try {
			...
			int firstCount = 0;
			String csvData;
			for (; rowIter.hasNext(); writeCSV(csvFileName,
					(new StringBuilder(String.valueOf(csvData.substring(0, csvData.length() - 1)).trim()))
							.append("\r\n").toString())) {
				csvData = "";
				...
				for (int i = 0; i < firstCount; i++) {
					csvData = String.valueOf(csvData)
							+ getCellData(myRow.getCell(i)).replace("\t", "").replace("\n", "").replace("\r", "")
	...
	private void writeCSV(String csvFileName, String csvData) throws Exception {
	...
	private String getXLSXCellData(XSSFCell myCell) throws Exception {
	...
	private static String getXLSXNumericValue(XSSFCell myCell) throws Exception {
		...
		if (DateUtil.isCellDateFormatted(myCell)) {
			...
			cellData = String.valueOf(cellData) + new SimpleDateFormat(OUTPUT_DATE_FORMAT).format(obj)
					+ CVS_SEPERATOR_CHAR;// new SimpleDateFormat(OUTPUT_DATE_FORMAT).format(
		} else {
			// cellData =
			// String.valueOf(cellData)+myCell.getNumericCellValue()+CVS_SEPERATOR_CHAR;
	...
	private static String getNumericValue(HSSFCell myCell) throws Exception {
		...
		if (DateUtil.isCellDateFormatted(myCell)) {
			...
			// cellData =
			// String.valueOf(cellData)+formatter.formatCellValue(myCell)+CVS_SEPERATOR_CHAR;
	...
	private static String getFormulaValue(HSSFCell myCell) throws Exception {
		String cellData = "";
		if (myCell.getCachedFormulaResultType() == CellType.STRING
				|| (myCell.getCachedFormulaResultType() == CellType.BOOLEAN)) {
			cellData = String.valueOf(cellData) + myCell.getRichStringCellValue() + CVS_SEPERATOR_CHAR;
		} else if (myCell.getCachedFormulaResultType() == CellType.NUMERIC) {
	...
	public void updateBatchTransaction(String tranId, String progressStatus, String errordesc, String progrssDesc,
			String loading) {
		...
		try {
			...
			if (!CollectionUtils.isEmpty(list)) {
				...
				error_records = list.stream()
						.filter(p -> "E".equalsIgnoreCase(p.getErrorStatus()) || "E".equals(p.getStatus())).count();
	...
	public void saveUploadTransactionData(FileUploadInputRequest res) {
		...
		try {
			...
			controlDetails = TransactionControlDetails.builder()
					...
					.lastUpdatedDate(new Date()).loadPercentage(null)
					...
					.typeId(Long.valueOf(0)).tranDate(new Date()).build();
		...
		try {
			...
			if (errorList.size() == 0 && errorList.isEmpty()) {
				...
				if (list.size() > 0) {
					...
					dropDownList = rateMasterServiceImpl.masterDiscreteApiCall(factorRateSaveReq,
							auth.replaceAll("Bearer ", "").split(",")[0]);
					...
					if (StringUtils.isNotBlank(discreateColumns)) {
						...
						log.info("Grouping the records block calling based on the discreate columns ||"
								+ discreateColumns);
						if (discreateColArr.length == 1) {
							...
							for (Entry<Object, Map<Object, Map<Object, List<FactorRateRawInsert>>>> entry1 : groupdata
									.entrySet()) {
							...
							Map<Object, Map<Object, Map<Object, Map<Object, List<FactorRateRawInsert>>>>> groupdata = list
									.stream()
									.collect(Collectors.groupingBy(getGroupFields().get(discreateColArr[0]), Collectors
											.groupingBy(getGroupFields().get(discreateColArr[1]), Collectors.groupingBy(
													getGroupFields().get(discreateColArr[2]),
													Collectors.groupingBy(getGroupFields().get(discreateColArr[3]))))));

							for (Entry<Object, Map<Object, Map<Object, Map<Object, List<FactorRateRawInsert>>>>> entry1 : groupdata
									.entrySet()) {
								Map<Object, Map<Object, Map<Object, List<FactorRateRawInsert>>>> d1 = entry1.getValue();
								for (Entry<Object, Map<Object, Map<Object, List<FactorRateRawInsert>>>> entry2 : d1
										.entrySet()) {
							...
							Map<String, Map<String, Map<String, Map<String, Map<String, Map<String, Map<String, Map<String, Map<String, Map<String, Map<String, Map<String, List<FactorRateRawInsert>>>>>>>>>>>>> groupData = list
									.stream()
									.collect(Collectors.groupingBy(getGroupFields().get(discreateColArr[0]), Collectors
											.groupingBy(getGroupFields().get(discreateColArr[1]), Collectors.groupingBy(
													getGroupFields().get(discreateColArr[2]),
													Collectors.groupingBy(getGroupFields().get(discreateColArr[3]),
															Collectors.groupingBy(
																	getGroupFields().get(discreateColArr[4]),
																	Collectors.groupingBy(
																			getGroupFields().get(discreateColArr[5]),
																			Collectors.groupingBy(
																					getGroupFields()
																							.get(discreateColArr[6]),
																					Collectors.groupingBy(
																							getGroupFields().get(
																									discreateColArr[7]),
																							Collectors.groupingBy(
																									getGroupFields()
																											.get(discreateColArr[8]),
																									Collectors
																											.groupingBy(
																													getGroupFields()
																															.get(discreateColArr[9]),
																													Collectors
																															.groupingBy(
																																	getGroupFields()
																																			.get(discreateColArr[10]),
																																	Collectors
																																			.groupingBy(
																																					getGroupFields()
																																							.get(discreateColArr[11]))))))))))))));
							...
							Map<String, Map<String, Map<String, Map<String, Map<String, Map<String, Map<String, Map<String, List<FactorRateRawInsert>>>>>>>>> groupData = list
									.stream()
									.collect(Collectors.groupingBy(getGroupFields().get(discreateColArr[0]), Collectors
											.groupingBy(getGroupFields().get(discreateColArr[1]), Collectors.groupingBy(
													getGroupFields().get(discreateColArr[2]),
							...
							Map<String, Map<String, Map<String, Map<String, Map<String, List<FactorRateRawInsert>>>>>> groupdata = list
									.stream()
									.collect(Collectors.groupingBy(getGroupFields().get(discreateColArr[0]), Collectors
											.groupingBy(getGroupFields().get(discreateColArr[1]), Collectors.groupingBy(
													getGroupFields().get(discreateColArr[2]),
							...
							Map<String, Map<String, Map<String, Map<String, Map<String, Map<String, List<FactorRateRawInsert>>>>>>> groupData = list
									.stream()
									.collect(Collectors.groupingBy(getGroupFields().get(discreateColArr[0]), Collectors
											.groupingBy(getGroupFields().get(discreateColArr[1]), Collectors.groupingBy(
													getGroupFields().get(discreateColArr[2]),
													Collectors.groupingBy(getGroupFields().get(discreateColArr[3]),
															Collectors.groupingBy(
																	getGroupFields().get(discreateColArr[4]),
																	Collectors.groupingBy(getGroupFields()
																			.get(discreateColArr[5]))))))));
							...
							Map<String, Map<String, Map<String, Map<String, Map<String, Map<String, Map<String, List<FactorRateRawInsert>>>>>>>> groupData = list
									.stream()
									.collect(Collectors.groupingBy(getGroupFields().get(discreateColArr[0]), Collectors
											.groupingBy(getGroupFields().get(discreateColArr[1]), Collectors.groupingBy(
													getGroupFields().get(discreateColArr[2]),
							...
							Map<String, Map<String, Map<String, Map<String, Map<String, Map<String, Map<String, Map<String, Map<String, List<FactorRateRawInsert>>>>>>>>>> groupData = list
									.stream()
									.collect(Collectors.groupingBy(getGroupFields().get(discreateColArr[0]), Collectors
											.groupingBy(getGroupFields().get(discreateColArr[1]), Collectors.groupingBy(
													getGroupFields().get(discreateColArr[2]),
											...
											h.forEach((l, m) -> {
												m.forEach((n, o) -> {
													o.forEach((p, q) -> {
														q.forEach((r, s) -> {
															s.forEach((t, u) -> {
																loadList.put(String.valueOf(uniqueId.getAndIncrement()),
																		u);
						...
						log.info("Grouping the records block completed based on the discreate columns ||"
								+ discreateColumns);

						List<List<FactorRateRawInsert>> data = loadList.entrySet().stream().map(p -> p.getValue())
	...
	private void callNonDiscreate(List<FactorRateRawInsert> list, String auth,
			Map<String, List<DropDownRes>> dropDownList) {
		try {
	...
	public void updateFactorRawRecords(FactorRateSaveReq req, List<Error> errors, String tranId,
			String discreateColumns) {
		try {
			List<FactorRateRawInsert> saveData = req.getFactorParams().stream().map(p -> {
				...
				entity.setErrorDesc(
						errors.size() > 0
								? print.toJson(errors).length() > 10000 ? print.toJson(errors).substring(0, 10000)
										: print.toJson(errors)
								: null);
	...
	public void insertFactorRecordsToMainTable(String tranId, String totalRecord) {
		...
		try {
			...
			if (!CollectionUtils.isEmpty(list)) {
				...
				try {
					parmasMapping = list.stream().map(from -> {
						try {
				...
				List<FactorParamsInsert> params = parmasMapping.stream()
						.map(d -> mapper.map(d, FactorParamsInsert.class)).collect(Collectors.toList());
	...
	public CommonRes doMainJob(String tranId) throws Exception {
		...
		try {
			...
			if (count == 0) {
				...
				JobParameters jobParameters = new JobParametersBuilder().addLong("time", System.currentTimeMillis())
						.addString("MainTable", tranId + "~" + count).toJobParameters();
				Job job = applicationContext.getBean("MainTableJob", Job.class);
	...
	public void updateMasterValidation(SpringBatchMapperResponse res) {
		try {
			Integer count = rawMasterRepository.updateAgencyCodeValidation(res.getTranId(), res.getInsuranceId(),
					res.getProductId());
