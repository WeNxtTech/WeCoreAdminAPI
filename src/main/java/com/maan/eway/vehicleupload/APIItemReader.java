...
public class APIItemReader extends JdbcCursorItemReader<FactorBatchRecordRes> implements ItemReader<FactorBatchRecordRes> {
	...
	public APIItemReader(DataSource dataSource, String request_ref_no, Integer fromId, Integer toId) {
	    ...
	    super(dataSource,
	            "SELECT FACTOR_TYPE_ID,SNO,COMPANY_ID,PRODUCT_ID,BRANCH_CODE,AGENCY_CODE,SECTION_ID,COVER_ID,SUB_COVER_ID,"
	            ...
	            + "PARAM_26,PARAM_27,PARAM_28,API_URL,EXCESS_AMOUNT,EXCESS_PERCENT,EXCESS_DESC "
	            + "FROM factor_rate_raw_master "
	            + "WHERE TRAN_ID = ? AND ERROR_STATUS != 'E'",
	            new FactorRateRowMapper());

	    setName("factorRateApiItemReader-" + request_ref_no);

	    setPreparedStatementSetter(new PreparedStatementSetter() {
	        @Override
	        public void setValues(PreparedStatement ps) throws SQLException {
	            ps.setString(1, request_ref_no);
	        }
	    });
    ...
    // FIXED: Made static to prevent memory leak issues within Batch Jobs
    public static class FactorRateRowMapper implements RowMapper<FactorBatchRecordRes> {
        @Override
        public FactorBatchRecordRes mapRow(ResultSet rs, int rowNum) throws SQLException {
            return FactorBatchRecordRes.builder()
